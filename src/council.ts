...
import { TraitSystem, TRAIT_NAME_TO_INDEX, TRAIT_COUNT } from "./traits.js";
import { DirectorRNN, DirectorRNNState, OUTPUT_DIM } from "./rnn.js";
...
export const SUB_AGENT_NAMES: string[] = Object.keys(DEFAULT_COUNCIL_WEIGHTS);
...
export class CognitiveCouncil {
  ...
  private weightVectors: Map<string, number[]>;
  ...
  computeCouncilModulation(traits: number[]): number[] | null {
    ...
    if (actValues.length === 0) return null;
...
export const DIRECTOR_ROLE_PRIORITY: DirectorRoleValue[] = [
...
export class DirectorPool {
  ...
  constructor(directorCount: number = DEFAULT_DIRECTOR_COUNT, rng?: Mulberry32) {
    if (directorCount < MIN_DIRECTOR_COUNT || directorCount > MAX_DIRECTOR_COUNT) {
      throw new Error(
        `directorCount must be between ${MIN_DIRECTOR_COUNT} and ` +
  ...
  getState(): DirectorPoolState {
    const directors: DirectorPoolState["directors"] = {};
    for (const [key, director] of this.directors) {
      directors[key] = {
  ...
  static fromState(state: DirectorPoolState): DirectorPool {
    ...
    const pool = Object.create(DirectorPool.prototype) as DirectorPool;
