...
import { SPEECH_ACTIONS } from "./actions.js";
import { CognitiveCouncil, DirectorPool } from "./council.js";
import { DecisionModel, BUDDY_ACTIONS } from "./decision.js";
import { Emotion, EmotionalSystem, } from "./emotional-state.js";
import { rollBones, Mulberry32 } from "./identity.js";
import { PerceptionMapper } from "./perception.js";
import { EvolutionTrigger, PersonalityEvolutionEngine, } from "./personality-evolution.js";
...
export class BuddyBrain {
    ...
    _sessionFrictionCount = 0;
    _sessionTestFails = 0;
    _sessionTestPasses = 0;
    _frustrationCount = 0;
    _challengeCount = 0;
    _prevMomentum = 0.0;
    constructor() {
    ...
    tick(toolEvent) {
        ...
        this._perception.updateFromToolEvent(toolEvent.tool_name ?? "", (toolEvent.tool_input ?? {}), toolEvent.success ?? true);
        // 2. Track test outcomes for evolution triggers
        const cmd = ((toolEvent.tool_input ?? {}).command ?? "");
        const cmdLower = cmd.toLowerCase();
        if (["pytest", "npm test", "cargo test", "jest"].some((t) => cmdLower.includes(t))) {
        ...
        // 5. Decay emotions
        this._emotionalStates = EmotionalSystem.decayEmotions(this._emotionalStates, this._tickCount);
        // 6. Apply personality shifts
        this._traits = this._evolutionEngine.applyShifts(this._traits, this._personalityShifts, this._tickCount);
        ...
        const emotionMods = this._emotionalSystem.computeTraitModifiers(this._emotionalStates, this._tickCount);
        const effectiveTraits = clip(this._traits.map((t, i) => t + emotionMods[i]), 0.0, 1.0);
        // 12. Decide action
        const result = this._decisionModel.decide(effectiveTraits, { tick: this._tickCount / 100.0 }, BUDDY_ACTIONS, undefined, // actionWeights
        undefined, // actionBiases
        councilMod ?? undefined, meanBias, this._rng);
        ...
        const expression = getExpression(actionName, dominantEmotion, councilActivations, perceptionVec[3]);
        // 15. Select speech
        let speech = null;
        if (SPEECH_ACTIONS.has(actionName)) {
            speech = selectSpeech(actionName, effectiveTraits, dominantEmotion, this._rng);
        }
        // 16. Get sprite frame
        const spriteFrame = getSpriteFrame(this._species, expression, this._tickCount);
    ...
    getCard() {
        ...
        return {
            ...
            adornments: checkAdornments(this._traits, this._traitsAtCreation, this._frustrationCount, this._challengeCount),
    ...
    loadMind(filePath, userId) {
        ...
        this._emotionalStates = (state.emotional_states ?? []).map((s) => ({
            emotion: s.emotion,
            intensity: s.intensity,
            valence: s.valence,
        ...
        this._personalityShifts = (state.personality_shifts ?? []).map((s) => ({
    ...
    // Private helpers
    // -----------------------------------------------------------------------
    _triggerEmotions(toolEvent) {
        const success = toolEvent.success ?? true;
        const toolName = toolEvent.tool_name ?? "";
        const cmd = ((toolEvent.tool_input ?? {}).command ?? "");
        const cmdLower = cmd.toLowerCase();
        const isTest = ["pytest", "npm test", "cargo test", "jest", "vitest"].some((t) => cmdLower.includes(t));
        if (isTest && success) {
            this._emotionalStates.push(this._emotionalSystem.addEmotion(Emotion.JOY, 0.6, 0.8, this._tickCount));
        }
        else if (isTest && !success) {
            this._emotionalStates.push(this._emotionalSystem.addEmotion(Emotion.FRUSTRATION, 0.5, -0.6, this._tickCount));
            this._frustrationCount += 1;
        }
        else if (!success) {
            this._emotionalStates.push(this._emotionalSystem.addEmotion(Emotion.WARINESS, 0.3, -0.3, this._tickCount));
        }
        // Editing code triggers mild satisfaction or excitement
        if (toolName === "Edit" || toolName === "Write") {
            if (success) {
                this._emotionalStates.push(this._emotionalSystem.addEmotion(Emotion.SATISFACTION, 0.3, 0.3, this._tickCount));
            }
        }
        // Reading/exploring triggers curiosity at lower threshold than novelty
        if (toolName === "Read" || toolName === "Glob" || toolName === "Grep") {
            this._emotionalStates.push(this._emotionalSystem.addEmotion(Emotion.CURIOSITY, 0.3, 0.3, this._tickCount));
        }
        // Bash commands trigger excitement (something is happening)
        if (toolName === "Bash" && success) {
            this._emotionalStates.push(this._emotionalSystem.addEmotion(Emotion.EXCITEMENT, 0.25, 0.3, this._tickCount));
        }
        ...
        if (vec[3] > 0.6) {
            ...
            this._emotionalStates.push(this._emotionalSystem.addEmotion(Emotion.CONTENTMENT, 0.3, 0.4, this._tickCount));
        }
        // Novelty triggers curiosity
        if (vec[5] > 0.6) {
            this._emotionalStates.push(this._emotionalSystem.addEmotion(Emotion.CURIOSITY, 0.5, 0.4, this._tickCount));
        }
    ...
    _checkEvolutionTriggers() {
        const vec = this._perception.getVector();
        // Sustained debugging: high pressure over many ticks
        if (vec[1] > 0.5 &&
            this._sessionTestFails > 10 &&
            this._tickCount > 10) {
            const shifts = this._evolutionEngine.createShift(EvolutionTrigger.SUSTAINED_DEBUGGING, this._tickCount);
        ...
        if (vec[5] > 0.6 && this._tickCount > 5) {
            const shifts = this._evolutionEngine.createShift(EvolutionTrigger.CREATIVE_EXPLORATION, this._tickCount, 0.02);
            this._personalityShifts.push(...shifts);
        }
        // Breakthrough: sudden momentum jump
        if (vec[3] - this._prevMomentum > 0.5) {
            const shifts = this._evolutionEngine.createShift(EvolutionTrigger.BREAKTHROUGH, this._tickCount);
            this._personalityShifts.push(...shifts);
            this._evolutionHistory.push({
        ...
        if (vec[10] > 0.5 && this._tickCount > 8) {
            const shifts = this._evolutionEngine.createShift(EvolutionTrigger.METHODICAL_TESTING, this._tickCount, 0.02);
            this._personalityShifts.push(...shifts);
        }
    }
    _computeReward(toolEvent) {
        if (toolEvent.success) {
            return 0.5;
        }
        return -0.3;
    ...
    _getDominantEmotion() {
        const active = this._emotionalStates.filter((s) => EmotionalSystem.isActive(s, this._tickCount));
        if (active.length === 0)
            return null;
        let strongest = active[0];
        let strongestIntensity = EmotionalSystem.currentIntensity(strongest, this._tickCount);
        for (let i = 1; i < active.length; i++) {
            const intensity = EmotionalSystem.currentIntensity(active[i], this._tickCount);
            if (intensity > strongestIntensity) {
                strongest = active[i];
                strongestIntensity = intensity;
            }
        }
        if (strongestIntensity < 0.1)
            return null;
