...
export const TOTAL_WEIGHTS = INPUT_DIM + // attention_mask: 14
...
// ---------------------------------------------------------------------------
function reshapeToMatrix(flat, rows, cols) {
    const m = [];
...
export class DirectorRNN {
    ...
    getWih() {
        ...
        return reshapeToMatrix(this._genome.slice(offset, offset + INPUT_DIM * HIDDEN_DIM), HIDDEN_DIM, INPUT_DIM);
    }
    getWhh() {
        const offset = INPUT_DIM + INPUT_DIM * HIDDEN_DIM;
        return reshapeToMatrix(this._genome.slice(offset, offset + HIDDEN_DIM * HIDDEN_DIM), HIDDEN_DIM, HIDDEN_DIM);
    }
    getWho() {
        const offset = INPUT_DIM + INPUT_DIM * HIDDEN_DIM + HIDDEN_DIM * HIDDEN_DIM;
        return reshapeToMatrix(this._genome.slice(offset, offset + HIDDEN_DIM * OUTPUT_DIM), OUTPUT_DIM, HIDDEN_DIM);
    }
    getBiasH() {
        const offset = INPUT_DIM +
    ...
    setAttentionMask(mask) {
        for (let i = 0; i < INPUT_DIM; i++)
            this._genome[i] = mask[i];
    }
    setWih(m) {
        const flat = flattenMatrix(m);
        const offset = INPUT_DIM;
        for (let i = 0; i < flat.length; i++)
            this._genome[offset + i] = flat[i];
    }
    setWho(m) {
        const flat = flattenMatrix(m);
        const offset = INPUT_DIM + INPUT_DIM * HIDDEN_DIM + HIDDEN_DIM * HIDDEN_DIM;
    ...
    // ----- REINFORCE learning -----
    learnFromOutcome(perception, biasProduced, outcomeSignal, lr = LEARNING_RATE) {
        ...
        const Wih = this.getWih();
        const hiddenGrad = new Array(HIDDEN_DIM);
        ...
        const wihTHiddenGrad = new Array(INPUT_DIM).fill(0);
        ...
        const sigGrad = new Array(INPUT_DIM);
        for (let i = 0; i < INPUT_DIM; i++) {
